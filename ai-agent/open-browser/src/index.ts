#!/usr/bin/env node
/**
 * Open Browser Agent CLI
 * 
 * Usage:
 *   open-browser-agent                    # Interactive mode
 *   open-browser-agent "search for..."     # Single query mode
 *   OPENAI_API_KEY=xxx open-browser-agent # With env vars
 * 
...
function loadConfig(): Config {
  ...
  if (!apiKey) {
    // Try config file
    const configPath = join(homedir(), '.open-agent', 'config.json');
    ...
    console.error('');
    console.error('Or create ~/.open-agent/config.json:');
    console.error('  {');
...
async function interactiveMode(agent: Agent): Promise<void> {
  console.log('🌐 Open Browser Agent');
